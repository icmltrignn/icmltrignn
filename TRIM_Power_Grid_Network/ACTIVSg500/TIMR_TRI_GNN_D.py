...
# Load data
# 1, adj processing
adj = pd.read_csv(path + "/ACTIVSg500_adj.csv",index_col=0)
adj = adj.values
adj = csr_matrix(adj)

# 2, features
bus_input = pd.read_csv(path + "/ACTIVSg500_feature_matrix.csv",index_col = 0)
node_features = bus_input.iloc[:,0:5]
node_features = csr_matrix(node_features)

# 3, label
z = pd.read_csv(path + "/ACTIVSg500_y.csv",index_col = 0).values
z = z.reshape((500,))
idx = np.arange(len(z))
label = z
...
idx_train = pd.read_csv(path + "/ACTIVSg500_idx_train.csv",header=0, index_col=0).values
idx_train = idx_train.reshape(-1,)

idx_val = pd.read_csv(path + "/ACTIVSg500_idx_val.csv",header=0, index_col=0).values
idx_val = idx_val.reshape(-1,)

idx_test = pd.read_csv(path + "/ACTIVSg500_idx_test.csv",header=0, index_col=0).values
...
sc_wasserstein_distance_dataset = pd.read_csv(path + "/ACTIVSg500_wasserstein_distance.csv", index_col= 0)
...
topo_features = (topo_normalization(sc_wasserstein_distance).dot(node_features_arr) + node_features_arr)/2.
