...
# Load data
# adj processing #
adj = pd.read_csv(path + "/Ill_BR_B_adj_network.csv",index_col=0)
adj = adj.values
adj = csr_matrix(adj)
...
# features #
bus_input = pd.read_csv(path + "/Ill_BR_B_feature_matrix.csv",index_col = 0)
node_features = bus_input.iloc[:,0:5]
node_features = csr_matrix(node_features)

# 3, label #
z = pd.read_csv(path + "/Ill_BR_B_y.csv",index_col = 0).values
z = z.reshape((200,))
idx = np.arange(len(z))
label = z
...
idx_train = pd.read_csv(path +"/idx_train.csv",header=0, index_col=0).values
idx_train = idx_train.reshape(-1,)

idx_val = pd.read_csv(path +"/idx_val.csv",header=0, index_col=0).values
idx_val = idx_val.reshape(-1,)

idx_test = pd.read_csv(path +"/idx_test.csv",header=0, index_col=0).values
...
il_wasserstein_distance_dataset = pd.read_csv("il_wasserstein_distance_mat_5_nKNN.csv", index_col= 0)
